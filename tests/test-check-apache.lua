...
require('tap')(function(test)
  ...
  test('check apache vhost', function(expect)
    local HOST, PORT = '127.0.0.1', 32501
    local url = string.format('http://%s:%s/server-status?auto', HOST, PORT)
    local ch = ApacheCheck:new({id='foo', period=30, details={url=url}})
    local response = fs.readFileSync('static/tests/checks/apache_server_status_vhost.txt')
    local server

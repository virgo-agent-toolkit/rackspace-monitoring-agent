...
The generation of bundles obeys the following rules.

1. Only Lua files are included ...except
2. All files in the root level "static" directory ...except
3. Files which don't match the bundle type.

Virgo automatically generates two bundles: one includes files found in tests and static/tests while the other does not.  This bundles will take the name of the final directory that was specified to configure plus the suffic "-bundle.zip" or "-bundle-test.zip".
